...
net = networkPlot(NetMatrix, weighted = NULL, n = 50, Title = "Authors' bibliographic coupling", label.n=15, type = "fruchterman", size = 10, cluster="walktrap", remove.multiple = TRUE, labelsize = 0.8)
```
...
#clean environment
rm(bib, net, NetMatrix, NetMatrix2, NetMatrix3)
```
...
gD<-delete.vertices(bib_plot, bad.vs) #exclude them from the graph

plot(bib_plot) #another hairball...
```
